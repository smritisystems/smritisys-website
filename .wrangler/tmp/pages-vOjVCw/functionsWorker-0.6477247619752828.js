...
// C:/Users/netma/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/_internal/utils.mjs
// @__NO_SIDE_EFFECTS__
...
// C:/Users/netma/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/perf_hooks/performance.mjs
...
// C:/Users/netma/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/@cloudflare/unenv-preset/dist/runtime/polyfill/performance.mjs
...
// C:/Users/netma/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/console.mjs
import { Writable } from "node:stream";

// C:/Users/netma/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/mock/noop.mjs
...
// C:/Users/netma/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/@cloudflare/unenv-preset/dist/runtime/node/console.mjs
...
// C:/Users/netma/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-console
globalThis.console = console_default;

// C:/Users/netma/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/process/hrtime.mjs
...
// C:/Users/netma/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/process/process.mjs
import { EventEmitter } from "node:events";

// C:/Users/netma/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/tty/read-stream.mjs
...
// C:/Users/netma/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/tty/write-stream.mjs
...
// C:/Users/netma/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/process/node-version.mjs
...
// C:/Users/netma/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/@cloudflare/unenv-preset/dist/runtime/node/process.mjs
...
// C:/Users/netma/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-process
...
async function getCustomerSession(request, env2) {
...
async function getStaffSession(request, env2) {
  const auth = request.headers.get("Authorization") || "";
  const token = auth.replace("Bearer ", "").trim();
  if (!token) return null;
  return await env2.DB.prepare(
...
__name(getStaffSession, "getStaffSession");
async function requireSuperAdmin(request, env2) {
  const session = await getStaffSession(request, env2);
  if (!session || session.status !== "active" || session.role !== "super_admin") return null;
  return session;
}
__name(requireSuperAdmin, "requireSuperAdmin");
async function handleAdminUsers(request, env2) {
  const session = await requireSuperAdmin(request, env2);
  ...
  await env2.DB.prepare(
    `UPDATE users
     ...
     WHERE id = ?`
  ).bind(body.name || null, body.role || null, body.status || null, userId).run();
  return json({ ok: true, message: "User updated" });
}
__name(handleAdminUsers, "handleAdminUsers");
async function handleProfile(request, env2) {
  const session = await getCustomerSession(request, env2);
  ...
  if (request.method === "GET") {
    ...
    return json({ ok: true, profile: profile3 });
  }
  const body = await request.json();
...
async function ensureDefaultAccounts(env2, customerId) {
  const defaults = [
    ["1000", "Cash", "asset"],
  ...
  const statements = defaults.map(([code, name, accountType]) => env2.DB.prepare(
...
async function handleAccounting(request, env2, resource) {
  const session = await getCustomerSession(request, env2);
  if (!session) return json({ ok: false, error: "Customer login required" }, 401);
  const customerId = session.account_id;
  await ensureDefaultAccounts(env2, customerId);
  if (resource === "accounts") {
    ...
    try {
      ...
      if (String(error3).includes("UNIQUE")) return json({ ok: false, error: "Account code already exists" }, 409);
      throw error3;
    }
  }
  ...
  if (simpleDocuments[resource]) {
    const config2 = simpleDocuments[resource];
    if (request.method === "GET") {
      const { results } = await env2.DB.prepare(
        `SELECT * FROM ${config2.table} WHERE customer_id = ? ORDER BY ${config2.date} DESC, id DESC`
      ).bind(customerId).all();
      return json({ ok: true, [resource]: results });
    }
    const body = await request.json();
    const required = resource === "notes" ? [config2.number, "note_type", "party_type", config2.date, "amount", "reason"] : [config2.number, config2.date, "amount", "payment_mode"];
    ...
    try {
      const columns = ["customer_id", config2.number, config2.contact, config2.reference, config2.date, "amount"];
      const values = [customerId, body[config2.number], body[config2.contact] || null, body[config2.reference] || null, body[config2.date], Number(body.amount) || 0];
      if (resource === "notes") {
      ...
      await env2.DB.prepare(`INSERT INTO ${config2.table} (${columns.join(", ")}) VALUES (${placeholders})`).bind(...values).run();
      return json({ ok: true, message: `${resource === "receipts" ? "Receipt" : resource === "payments" ? "Supplier payment" : "Note"} saved` }, 201);
    } catch (error3) {
      if (String(error3).includes("UNIQUE")) return json({ ok: false, error: "Document number already exists" }, 409);
      throw error3;
    }
  }
  if (resource === "invoices" || resource === "purchases") {
    if (request.method === "GET") {
      const table4 = resource === "invoices" ? "accounting_invoices" : "accounting_purchases";
      const orderField = resource === "invoices" ? "invoice_date" : "purchase_date";
      const { results } = await env2.DB.prepare(
        `SELECT * FROM ${table4} WHERE customer_id = ? ORDER BY ${orderField} DESC, id DESC`
      ).bind(customerId).all();
      return json({ ok: true, [resource]: results });
    }
    const body = await request.json();
    ...
    const table3 = isInvoice ? "accounting_invoices" : "accounting_purchases";
    ...
    try {
      const parent = await env2.DB.prepare(
        `INSERT INTO ${table3} (customer_id, ${numberColumn}, ${partyColumn}, ${isInvoice ? "buyer_email, " : ""}${dateColumn}, ${isInvoice ? "due_date, " : ""}subtotal, tax_amount, total_amount)
      ...
      const itemStatements = normalizedItems.map((item) => env2.DB.prepare(
      ...
      const accountRows = await env2.DB.prepare(
      ...
      await env2.DB.batch(itemStatements);
...
async function handleTickets(request, env2) {
  const session = await getCustomerSession(request, env2);
  if (!session) return json({ ok: false, error: "Customer login required" }, 401);
  if (request.method === "GET") {
    const { results } = await env2.DB.prepare(
  ...
  const result = await env2.DB.prepare(
    `INSERT INTO support_tickets (customer_id, subject, description, priority)
     VALUES (?, ?, ?, ?)`
  ).bind(session.account_id, subject, description, ticketPriority).run();
  return json({ ok: true, ticket_id: result.meta.last_row_id }, 201);
}
__name(handleTickets, "handleTickets");
async function handleDemo(request, env2) {
  const body = await request.json();
...
async function handleSignup(request, env2) {
  const body = await request.json();
  ...
  try {
    if (type === "user") {
      ...
      ).bind(email.toLowerCase(), password_hash, name || null, "staff").run();
    } else {
      await env2.DB.prepare(
...
async function handleListDemos(request, env2) {
  const session = await requireSuperAdmin(request, env2);
  if (!session) return json({ ok: false, error: "Super admin access required" }, 403);
  const { results } = await env2.DB.prepare(
...
async function onRequest(context2) {
  ...
  try {
    if (path === "demo" && request.method === "POST") return await handleDemo(request, env2);
    if (path === "signup" && request.method === "POST") return await handleSignup(request, env2);
    if (path === "login" && request.method === "POST") return await handleLogin(request, env2);
    if (path === "me" && request.method === "GET") return await handleMe(request, env2);
    if (path === "profile" && ["GET", "PUT"].includes(request.method)) return await handleProfile(request, env2);
    if (path === "tickets" && ["GET", "POST"].includes(request.method)) return await handleTickets(request, env2);
    if (path === "accounting/accounts" && ["GET", "POST"].includes(request.method)) return await handleAccounting(request, env2, "accounts");
    if (path === "accounting/contacts" && ["GET", "POST"].includes(request.method)) return await handleAccounting(request, env2, "contacts");
    if (path === "accounting/invoices" && ["GET", "POST"].includes(request.method)) return await handleAccounting(request, env2, "invoices");
    if (path === "accounting/purchases" && ["GET", "POST"].includes(request.method)) return await handleAccounting(request, env2, "purchases");
    if (path === "accounting/receipts" && ["GET", "POST"].includes(request.method)) return await handleAccounting(request, env2, "receipts");
    if (path === "accounting/payments" && ["GET", "POST"].includes(request.method)) return await handleAccounting(request, env2, "payments");
    if (path === "accounting/notes" && ["GET", "POST"].includes(request.method)) return await handleAccounting(request, env2, "notes");
    if (path === "accounting/ledger" && request.method === "GET") return await handleAccounting(request, env2, "ledger");
    if (path === "demos" && request.method === "GET") return await handleListDemos(request, env2);
    if (path === "admin/users" && request.method === "GET") return await handleAdminUsers(request, env2);
    if (path.startsWith("admin/users/") && request.method === "PATCH") {
      request.params = { id: path.split("/")[2] };
      return await handleAdminUsers(request, env2);
    }
...
// ../.wrangler/tmp/pages-vOjVCw/functionsRoutes-0.027567311066015288.mjs
var routes = [
  {
    routePath: "/api/:path*",
    mountPath: "/api",
